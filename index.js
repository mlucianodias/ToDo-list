...
function makeTodoItemFromData(data) {
  ...
  if (!todoItem.owner) errors.push({ field: 'owner', message: 'Proprietário não especificado' });
  if (!todoItem.name) errors.push({ field: 'name', message: 'Título da tarefa não especificado' });
...
createServer(async (req, res) => {
  ...
  .on('listening', () => {
    console.log(`O servidor TODO está em execução. Você pode usá-lo em http://localhost:${PORT}`);
    console.log('Pressione CTRL+C para parar o servidor');
    console.log('Métodos Disponíveis:');
    console.log(`GET ${URI_PREFIX} - obter lista de tarefas, query parâmetro owner filtra por proprietário`);
    console.log(`POST ${URI_PREFIX} - criar uma tarefa, você precisa passar um objeto no corpo da solicitação { name: string, owner: string, done?: boolean }`);
    console.log(`GET ${URI_PREFIX}/{id} - obter uma tarefa por ID`);
    console.log(`PATCH ${URI_PREFIX}/{id} - alterar uma tarefa por ID, você precisa passar um objeto no corpo da solicitação { name?: string, owner?: string, done?: boolean }`);
